...
class LimitPaymentGatewaysExample {}
...
class LimitPaymentGatewaysTest extends WC_Calypso_Bridge_Test {
	...
	public function test_does_not_limit_on_biz() {
		...
		$this->assertEquals(
			[ new LimitPaymentGatewaysExample() ],
			wc_calypso_bridge_limit_payment_gateways( [ new LimitPaymentGatewaysExample(), new \stdClass() ] )
		);
	}
	...
	public function test_does_not_limit_on_ecom() {
		...
		$this->assertEquals(
			[ new LimitPaymentGatewaysExample(), new \stdClass() ],
			wc_calypso_bridge_limit_payment_gateways( [ new LimitPaymentGatewaysExample(), new \stdClass() ] )
		);
	}
}
